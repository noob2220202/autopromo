...
async def verify_and_confirm(session: AsyncSession, tx_id: str, telegram_id: int) -> dict:
    ...
    if not registered_addresses:
        raise PaymentVerificationError("먼저 결제에 사용할 지갑 주소를 등록해주세요.")
    ...
    if from_address not in registered_addresses:
        raise PaymentVerificationError("보낸 주소가 등록된 지갑 주소와 일치하지 않습니다.")
    if abs(amount - PRO_PLAN_USDT_PRICE) > 1e-6:
        raise PaymentVerificationError(f"금액이 {PRO_PLAN_USDT_PRICE} USDT와 일치하지 않습니다.")
